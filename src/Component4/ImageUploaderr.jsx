...
const ImageUploader = () => {
    ...
    const handleFileUpload = (event) => {
        ...
        if (unsupportedFiles.length > 0) {
            const unsupportedFormats = unsupportedFiles
                ...
                .join(", ");
            const errorMessage = `Unsupported file format ${unsupportedFormats}`;
    ...
    //     if (compressedImage) {
    //         const zip = new window.XMLHttpRequest();
    //         const imageFileName = "compressed_image";

    //         zip.open("GET", compressedImage, true);
    //         zip.responseType = "blob";

    //         zip.onload = function () {
    //             const blob = zip.response;
    //             const link = document.createElement("a");
